...
class G4fAskMod(loader.Module):
    ...
    async def ask(self, message: Message):
        ...
        try:
            ...
            await utils.answer(msg, self.strings("missing_dep"))
        except Exception as e:
            error_name = type(e).__name__
            if "request" in error_name.lower():
                logger.exception("Request error from g4f")
                await utils.answer(msg, self.strings("request_error").format(model, str(e)))
            else:
                logger.exception("Failed to get response from AI")
                await utils.answer(msg, self.strings("unexpected_error").format(model, str(e))) 
